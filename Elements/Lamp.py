...
class Lamp:
        ...
        for wire in self.wires_connected:
            if wire.powered:
                if wire.element1 == self:
                    self.state[wire.joint1] = 1
                elif wire.element2 == self:
                    self.state[wire.joint2] = 1
    ...
    def draw(self, position_x, position_y):
        ...
        if self.state[0] == 1 or self.state[1] == 1 or self.state[2] == 1 or self.state[3] == 1:
        ...
        if self.state[0] == 1:
            self.screen.blit(pygame.transform.rotate(self.joint1, 180), pygame.Rect(on_screen_x + 17, on_screen_y - 8, 15, 15))
        else:
            self.screen.blit(pygame.transform.rotate(self.joint0, 180), pygame.Rect(on_screen_x + 17, on_screen_y - 8, 15, 15))
        if self.state[1] == 1:
            self.screen.blit(pygame.transform.rotate(self.joint1, 270), pygame.Rect(on_screen_x - 8, on_screen_y + 17, 15, 15))
        else:
            self.screen.blit(pygame.transform.rotate(self.joint0, 270), pygame.Rect(on_screen_x - 8, on_screen_y + 17, 15, 15))
        if self.state[2] == 1:
