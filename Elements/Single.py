...
class Single:
    def __init__(self, screen, pos_x, pos_y):
        self.image = pygame.image.load("Textures/Pojedynczy.png")
        self.screen = screen
        self.pos_x = pos_x
        self.pos_y = pos_y
        self.state = [0, 0]
    ...
    def clicked(self, position_x, position_y):
        if self.pos_x * 40 - 5 + 17 < position_x < self.pos_x * 40 - 5 + 32 and self.pos_y * 40 - 5 - 7 < position_y < self.pos_y * 40 - 5 + 8:
            return self.pos_x * 40 - 5 + 25, self.pos_y * 40 - 2, self, 1
    ...
    def draw(self, position_x, position_y):
        ...
        if self.state[1] == 1:
            self.screen.blit(self.joint1, pygame.Rect(on_screen_x + 17, on_screen_y - 5, 15, 15))
        else:
            self.screen.blit(self.joint0, pygame.Rect(on_screen_x + 17, on_screen_y - 5, 15, 15))